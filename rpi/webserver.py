...
def run_webserver(collection: Collection):
    ...
    def get_raw_content_for_web_page():
    ...
    async def handle_post(request: web.Request) -> web.Response:
        ...
        sat.ip = request.remote
        if not sat.has_coords() and sat.addr == "58:BF:25:93:7E:84": # ESP B
            # sat.room = "Living"
            # sat.set_coords(1050,20)
            sat.room = "Klaslokaal"
            sat.set_coords(500,140)
        elif not sat.has_coords() and sat.addr == "58:BF:25:93:7C:88": # New C
            # sat.room = "Living"
            # sat.set_coords(1500,20)
            sat.room = "Klaslokaal"
            sat.set_coords(500,690)
        elif not sat.has_coords() and sat.addr == "58:BF:25:93:7E:78": # ESP A
            # sat.room = "Living"
            # sat.set_coords(1500,750)
            sat.room = "Klaslokaal"
            sat.set_coords(40,50)
        elif not sat.has_coords() and sat.addr == "58:BF:25:93:7D:20": # ESP D
            # sat.room = "Living"
            # sat.set_coords(1500,450)
            sat.room = "Klaslokaal"
            sat.set_coords(500,140)
    ...
    app.router.add_get(
        '/raw', lambda request: web.Response(text=get_raw_content_for_web_page()))
