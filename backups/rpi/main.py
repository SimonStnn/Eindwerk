...
from const import CONNECTIONS, HOST_IP, WEBSOCKET_PORT, WEBSERVER_PORT
...
#####################################################################################################################################
async def run_websocket_server(collection: Collection, stop_event: threading.Event) -> asyncio.AbstractEventLoop:
...
# heyy simon hoe geet het met je
    ...
    try:
        ...
        return await websocket_server.wait_closed()
    except Exception:
        pass


def run_web_server(collection: Collection):
    ...
    def get_raw_content_for_web_page(request: web.Request) -> web.Response:
        ...
        col_sats = collection.dic["sats"]
        # Format collection and serialise
        json_collection = format_dict(dict(collection.dic))
    ...
    async def handle_post(request: web.Request) -> web.Response:
        ...
        data = collection.decode_incoming_satellite_data(data)

        data_addr = data["sat"]["addr"]
        collection.add_satellite_to_collection(data_addr, data)

        devices = collection.calculate_devices_positions()
        collection.add_device_to_collection(devices)
        ...
        collection.broadcast_to_all_clients()
        return web.Response()
    ...
    app.router.add_get('/raw', lambda request : web.Response(text=get_raw_content_for_web_page(request)))
...
if __name__ == "__main__":
    ...
    websocket_thread = threading.Thread(
        target=asyncio.run, args=[
            run_websocket_server(shared_collection, stop_event)],
